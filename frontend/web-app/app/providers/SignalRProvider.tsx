...
export default function SignalRProvider({ children }: Props) {
    ...
    const handleFinished = useCallback((finishedAuction: AuctionFinished) => {
        const auction = getDetailsViewData(finishedAuction.auctionId);
        return toast.promise(auction, {
            loading: 'Loading',
            success: (auction) => 
                <ActionFinishedToast finishedAuction={finishedAuction} auction={auction} />,
    ...
    const handleAuctionCreated = useCallback((auction: Auction) => {
        if (user?.username === auction.seller) {
            return toast(<ActionCreatedToast auction={auction} />, {
                duration: 10000,
            });
        }
    }, [user?.username]);
    ...
    useEffect(() => {
        if (!connection.current) {
            ...
            connection.current.on("AuctionFinished", handleFinished);   
    ...
    }, [setCurrentPrice, handleBidPlaced, handleAuctionCreated, handleFinished]);
